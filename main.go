...
func usage() {
	fmt.Println("Usage: " + os.Args[0] + " -ip [ip] -user [user] -port [port] -pass [pass]")
	fmt.Printf("        -ip: 不传则脚本进入交互输入模式, 等于什么参数都没传; -user: 不传则默认所有ip user为root; -port: 不传则默认所有ip port为22; -pass: %s\n\n",
		color.YellowString("此选项仅供被其他脚本调用时传参, 手动运行脚本时建议不传, 脚本会提示输入密码"))
	flag.PrintDefaults()
	os.Exit(0)
}
...
func init() {
	...
	flag.StringVar(&passStr, "pass", "", `server password, 多个password空格隔开, 和ip按顺序匹配, 匹配数不足用最后一个, 例如: -pass "pass1 pass2"`)
	flag.Usage = usage
}
